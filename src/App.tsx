...
// Important : On réutilise ton fichier pocketbase.ts
...
const App = () => {
  ...
  // Fonction de scroll corrigée pour marcher avec HashRouter
  ...
  // VRAIE fonction d'envoi (PocketBase + Cal.com)
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    setIsSubmitting(true);

    try {
      // 1. On sauvegarde d'abord le lead dans PocketBase
      await pb.collection('demandes').create({
        ...formData,
        pack: selectedPack || 'Aucun pack sélectionné',
        email: 'client@contact.fr' // Champ technique requis par PB si email obligatoire
      });
      ...
      if (selectedPack.includes("Studio")) {
        ...
        eventSlug = "edl-maison"; // 2h
      } else {
        eventSlug = "edl-appart"; // 1h
      }

      // 3. Construction de l'URL avec pré-remplissage
      ...
      const confirm = window.confirm("✅ Demande enregistrée ! Voulez-vous choisir votre créneau maintenant ?");
